<section>
            ...
            <div class="flex-col space-y-4">
                <x-table>
                    <x-slot name="head">
                        ...
                        <x-table.heading sortable multi-column wire:click="sortBy('title')" :direction="$sorts['title'] ?? null" class="w-full">Nama Event</x-table.heading>
                    </x-slot>
                    ...
                    <x-slot name="body">
                        ...
                        <x-table.row class="bg-cool-gray-200" wire:key="row-message">
                            <x-table.cell colspan="9">
                                @unless ($selectAll)
                                <div>
                                    <span>You have selected <strong>{{ $data->count() }}</strong> Event, do you want to select all <strong>{{ $data->total() }}</strong>?</span>
                        ...
                        <x-table.row>
                            <x-table.cell colspan="8">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <span class="font-medium py-8 text-cool-gray-400 text-xl">No Event found...</span>
                                </div>
                            </x-table.cell>
                        </x-table.row>
                        @endforelse
    ...
    <form wire:submit.prevent="save">
        <x-modal.dialog wire:model.defer="showEditModal">
            <x-slot name="title">Create/Edit Event</x-slot>

            <x-slot name="content">
                <x-input.group :inline="'true'" for="title" label="Event" :error="$errors->first('request.title')">
                    <x-input.text type="title" wire:model="request.title" id="title" placeholder="Event" />
                </x-input.group>
            </x-slot>
